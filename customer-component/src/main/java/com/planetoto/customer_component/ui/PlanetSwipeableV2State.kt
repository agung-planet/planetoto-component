 ...
 *
 * When a swipe is detected, the offset of the [PlanetSwipeableV2State] will be updated with the swipe
 * delta. You should use this offset to move your content accordingly (see [Modifier.offset]).
 * When the swipe ends, the offset will be animated to one of the anchors and when that anchor is
 * reached, the value of the [PlanetSwipeableV2State] will also be updated to the value corresponding to
 * the new anchor.
 *
 * Swiping is constrained between the minimum and maximum anchors.
 *
 * @param state The associated [PlanetSwipeableV2State].
 * @param orientation The orientation in which the swipeable can be swiped.
 * @param enabled Whether this [planetSwipeableV2] is enabled and should react to the user's input.
 * @param reverseDirection Whether to reverse the direction of the swipe, so a top to bottom
 * swipe will behave like bottom to top, and a left to right swipe will behave like right to left.
 * @param interactionSource Optional [MutableInteractionSource] that will passed on to
 * the internal [Modifier.draggable].
 */
internal fun <T> Modifier.planetSwipeableV2(
    state: PlanetSwipeableV2State<T>,
...
/**
 * Define anchor points for a given [PlanetSwipeableV2State] based on this node's layout size and update
 * the state with them.
 *
 * @param state The associated [PlanetSwipeableV2State]
 * @param possibleValues All possible values the [PlanetSwipeableV2State] could be in.
 ...
 */
internal fun <T> Modifier.planetSwipeAnchors(
    state: PlanetSwipeableV2State<T>,
...
class PlanetSwipeableV2State<T>(
    initialValue: T,
    internal val animationSpec: AnimationSpec<Float> = PlanetSwipeableV2Defaults.AnimationSpec,
    internal val confirmValueChange: (newValue: T) -> Boolean = { true },
    internal val positionalThreshold: Density.(totalDistance: Float) -> Float =
        PlanetSwipeableV2Defaults.PositionalThreshold,
    internal val velocityThreshold: Dp = PlanetSwipeableV2Defaults.VelocityThreshold,
) {
    ...
    internal val swipeDraggableState = object : DraggableState {
        private val dragScope = object : DragScope {
            override fun dragBy(pixels: Float) {
                this@PlanetSwipeableV2State.dispatchRawDelta(pixels)
            }
        }
        ...
        override fun dispatchRawDelta(delta: Float) {
            this@PlanetSwipeableV2State.dispatchRawDelta(delta)
        }
    }

    /**
     * The current value of the [PlanetSwipeableV2State].
     */
     ...
     * [planetSwipeableV2] modifier is attached to. These are the phases:
    ...
    /**
     * Swipe by the [delta], coerce it in the bounds and dispatch it to the [PlanetSwipeableV2State].
     *
     * @return The delta the consumed by the [PlanetSwipeableV2State]
     */
    ...
    companion object {
        /**
         * The default [Saver] implementation for [PlanetSwipeableV2State].
         */
        ...
        ) = Saver<PlanetSwipeableV2State<T>, T>(
            save = { it.currentValue },
            restore = {
                PlanetSwipeableV2State(
                    initialValue = it,
...
/**
 * Create and remember a [PlanetSwipeableV2State].
 *
...
@Composable
@ExperimentalMaterial3Api
internal fun <T : Any> rememberCobaSwipeableV2State(
    initialValue: T,
    animationSpec: AnimationSpec<Float> = PlanetSwipeableV2Defaults.AnimationSpec,
    confirmValueChange: (newValue: T) -> Boolean = { true }
): PlanetSwipeableV2State<T> {
    return rememberSaveable(
        initialValue, animationSpec, confirmValueChange,
        saver = PlanetSwipeableV2State.Saver(
            animationSpec = animationSpec,
            confirmValueChange = confirmValueChange,
            positionalThreshold = PlanetSwipeableV2Defaults.PositionalThreshold,
            velocityThreshold = PlanetSwipeableV2Defaults.VelocityThreshold
        ),
    ) {
        PlanetSwipeableV2State(
 ...
 * anchor that needs to be reached for [PlanetSwipeableV2State] to settle to the next closest anchor.
 *
 ...
 * [PlanetSwipeableV2State] to settle to the next closest anchor.
 *
...
/**
 * Contains useful defaults for [planetSwipeableV2] and [PlanetSwipeableV2State].
 */
@Stable
internal object PlanetSwipeableV2Defaults {
    /**
     * The default animation used by [PlanetSwipeableV2State].
     */
    val AnimationSpec = SpringSpec<Float>()

    /**
     * The default velocity threshold (1.8 dp per millisecond) used by [rememberCobaSwipeableV2State].
     */
    val VelocityThreshold: Dp = 125.dp

    /**
     * The default positional threshold (56 dp) used by [rememberCobaSwipeableV2State]
     */
     ...
     * delegate to [PlanetSwipeableV2State].
     *
     * @param state The [PlanetSwipeableV2State] the change handler will read from
...
internal fun interface AnchorChangeHandler<T> {
    /**
     * Callback that is invoked when the anchors have changed, after the [PlanetSwipeableV2State] has been
