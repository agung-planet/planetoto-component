...
open class PlanetSwipeableState<T>(
    initialValue: T,
     ...
     * [planetSwipeable] is currently settled. If a swipe or animation is in progress, this corresponds
     * the last anchor at which the [planetSwipeable] was settled before the swipe or animation started.
     */
    ...
    /**
     * The current position (in pixels) of the [planetSwipeable].
     *
    ...
    /**
     * The amount by which the [planetSwipeable] has been swiped past its bounds.
     */
     ...
     *
     * If a swipe is in progress, this is the value that the [planetSwipeable] would animate to if the
    ...
    /**
     * Information about the ongoing swipe or animation, if any. See [PlanetSwipeProgress] for details.
     *
     * If no swipe or animation is in progress, this returns `SwipeProgress(value, value, 1f)`.
     */
    
    val progress: PlanetSwipeProgress<T>
        get() {
            ...
            return PlanetSwipeProgress(from, to, fraction)
        }

    /**
     * The direction in which the [planetSwipeable] is moving, relative to the current [currentValue].
     *
     ...
     * [velocity]. Fling with settling [planetSwipeable] will always consume all the velocity provided
     * since it will settle at the anchor.
     *
     * In general cases, [planetSwipeable] flings by itself when being swiped. This method is to be
     * used for nested scroll logic that wraps the [planetSwipeable]. In nested scroll developer may
    ...
    /**
     * Force [planetSwipeable] to consume drag delta provided from outside of the regular [planetSwipeable]
     * gesture flow.
     *
     ...
     *
     * In general cases, [planetSwipeable] drags by itself when being swiped. This method is to be
     * used for nested scroll logic that wraps the [planetSwipeable]. In nested scroll developer may
...
/**
 * Collects information about the ongoing swipe or animation in [planetSwipeable].
 *
...
@ExperimentalMaterialApi
internal fun <T> Modifier.planetSwipeable(
    state: PlanetSwipeableState<T>,
    anchors: Map<Float, T>,
...
internal val <T> PlanetSwipeableState<T>.PreUpPostDownNestedScrollConnection: NestedScrollConnection
