...
@Composable
fun PlanetSideDialog(
    sideDialogContent: @Composable ColumnScope.() -> Unit,
    modifier: Modifier = Modifier,
    sideDialogState: PlanetSideDialogState = rememberPlanetSideDialogState(DrawerValue.Closed),
...
@Stable
class PlanetSideDialogState(initialValue: DrawerValue) {
    internal val swipeableState = PlanetSwipeableState(
     ...
     */
    suspend fun open() = animateTo(DrawerValue.Open, PlanetAnimationSpec)
     ...
     */
    suspend fun close() = animateTo(DrawerValue.Closed, PlanetAnimationSpec)
    ...
    companion object {
        /**
         * The default [Saver] implementation for [PlanetSideDialogState].
         */
        fun Saver(confirmStateChange: (DrawerValue) -> Boolean) =
            Saver<PlanetSideDialogState, DrawerValue>(
                save = { it.currentValue },
                restore = { PlanetSideDialogState(it) }
            )
    }
}
...
@Composable
fun rememberPlanetSideDialogState(
    initialValue: DrawerValue,
    confirmStateChange: (DrawerValue) -> Boolean = { true }
): PlanetSideDialogState {
    return rememberSaveable(saver = PlanetSideDialogState.Saver(confirmStateChange)) {
        PlanetSideDialogState(initialValue)
    }
}

// this is taken from the DrawerLayout's DragViewHelper as a min duration.
private val PlanetAnimationSpec = TweenSpec<Float>(durationMillis = 256)
private val DrawerVelocityThreshold = 400.dp

/**
 * Contains useful defaults for [planetSwipeable] and [SwipeableState].
 */
object PlanetSwipeableDefaults {
    /**
